...
import { ConfiguredPayload, ConfiguredPayloadFile, ConfiguredPayloadJson, ConfiguredPayloadText } from "../payload/ConfiguredPayloads";
...
export async function loadPayload(payloadPath: string): Promise<ConfiguredPayload> {
    return PayloadConfig.toPayload(payloadPath, await _loadPayloadConfig(payloadPath));
}
