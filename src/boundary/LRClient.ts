...
export default class LRClient {
    ...
    constructor(
        loggerConfig: LRCLoggerConfig = new LRCLoggerConfig({}),
    ...
    async send(
        ...
        const variables = VariableMerger.mergeVariables(
            localVariables,
            endpoint,
            env
        );
        ...
        for (const key in headers) {
            if (Object.prototype.hasOwnProperty.call(headers, key)) {
                const resolvedValue = headers[key].resolve(
        ...
        // Add authentication if anywhere defined
        const auth = AuthMerger.mergeAuth(endpoint, env);
        if (auth) {
            request = await auth.sign(request, variables.variableStore);
        }
