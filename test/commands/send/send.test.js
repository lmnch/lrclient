...
describe('send-200', () => {
  ...
  before(() => {
    fetchMock((u) => u == 'https://test.url/start', m => m == "GET").status(200, "Ok").json({ test: "json" });
  })
...
describe("send-403", () => {
  before(() => {
    fetchMock(u => u == 'https://test.url/start', m => m == "GET").status(403, "Forbidden").json({});
  })
...
describe("send-upload-post-200", () => {
  ...
  before(() => {
    fetchMock(u => u == 'https://test.url/upload', m => m == "POST", undefined, b => JSON.parse(b).test.includes(42)).status(200, "Ok").json({ test: "json" });
  })

  test
    .stdout()
    .command(['send', 'test/resources/collections/url-upload-post.json', '-v', 'bearerToken: 123', '-v', 'value3: 42'])
    .it('[test-env|url-upload-post|test-upload-payload] should upload a json payload with variable', ctx => {
      const output = ctx.stdout.split("\n");
